...
router.get('/assignment-requests', adminAuth, async (req, res) => {
  try {
    const AgentCustomer = require('../models/AgentCustomer');
    
    const pendingRequests = await AgentCustomer.find({
      status: 'pending',
      assignmentType: 'agent_request'
    })
    .populate('customerId', 'username email')
    .populate('agentId', 'name agentId')
    .populate('requestedBy', 'name agentId')
    .sort({ requestedAt: -1 });
...
router.post('/assignment-requests/:id/approve', adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (!request) {
      return res.status(404).json({ error: 'Assignment request not found or already processed' });
    }
    ...
    if (existingAssignment) {
      return res.status(400).json({ error: 'Customer is already assigned to another agent' });
    }
    ...
    request.reviewNotes = notes || 'Approved';
    request.assignedBy = adminId;

    await request.save();

    // Populate response
    await request.populate('customerId', 'username email');
    await request.populate('agentId', 'name agentId');
...
router.post('/assignment-requests/:id/decline', adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { notes } = req.body;
    const adminId = req.admin._id;
    ...
    request.reviewedBy = adminId;
    request.reviewedAt = new Date();
    request.reviewNotes = notes || 'Declined';
    ...
    res.json({
      ...
      request: {
        ...
        agent: request.agentId,
        status: request.status,
        declinedBy: request.reviewedBy,
        declinedAt: request.reviewedAt,
        notes: request.reviewNotes
      }
    });
  } catch (error) {
    console.error('Error declining assignment request:', error);
    res.status(500).json({ error: 'Failed to decline assignment request' });
  }
});
