...
// Agent assignment request system
// Request assignment to a customer
router.post('/assignment-request', agentAuth, async (req, res) => {
  try {
    ...
    // Check if there's already an active assignment or pending request
    const existingAssignment = await AgentCustomer.findOne({
      customerId: customerId,
      $or: [
        { status: 'active' },
        { status: 'pending', requestedBy: agentId }
      ]
    });

    if (existingAssignment) {
      if (existingAssignment.status === 'active') {
        return res.status(400).json({ error: 'Customer is already assigned to an agent' });
      }
      if (existingAssignment.status === 'pending') {
        return res.status(400).json({ error: 'You already have a pending request for this customer' });
      }
    }

    // Create assignment request
    const assignmentRequest = new AgentCustomer({
      customerId: customerId,
      agentId: agentId,
      requestedBy: agentId,
      assignedBy: agentId, // Will be updated when approved
      assignmentType: 'agent_request',
      status: 'pending'
    });
    ...
    // Populate response
    await assignmentRequest.populate('customerId', 'username email');
    await assignmentRequest.populate('agentId', 'name agentId');

    res.status(201).json({
      success: true,
      message: 'Assignment request submitted successfully',
      request: {
        _id: assignmentRequest._id,
        customer: assignmentRequest.customerId,
        agent: assignmentRequest.agentId,
...
// Get agent's assignment requests
router.get('/assignment-requests', agentAuth, async (req, res) => {
  try {
    const agentId = req.agent._id;
    const AgentCustomer = require('../models/AgentCustomer');
    
    const requests = await AgentCustomer.find({
      requestedBy: agentId,
...
router.delete('/assignment-request/:requestId', agentAuth, async (req, res) => {
  try {
    ...
    const request = await AgentCustomer.findOne({
      _id: requestId,
      requestedBy: agentId,
      status: 'pending'
    });
    ...
    await AgentCustomer.deleteOne({ _id: requestId });
